...
function renderPricingTable() {
  ...
  tbody.innerHTML = cmsData.pricing.map((p, idx) => `
    <tr>
      ...
      <td>${p.Label || '-'}</td>
      <td>${p.Featured === 'Ya' ? '<span class="status-badge" style="background:#FEF3C7; color:#92400E;">Featured</span>' : 'Tidak'}</td>
...
function openPricingModal() {
  const nama = prompt("Nama Paket (contoh: EXCLUSIVE):");
  if (!nama) return;
  const harga = prompt("Harga Paket (angka saja, contoh: 249000):", "249000");
  const fitur = prompt("Daftar Fitur (pisahkan dengan baris baru / enter):", "Semua Fitur Premium\nCustom Domain Sendiri\nVideo Teaser 30 Detik");
  
  const newPkg = {
    ID: `PRC-${Date.now().toString().slice(-4)}`,
    Nama: nama,
    Harga: Number(harga) || 199000,
    Fitur: fitur || "",
    Label: "Spesial",
    Featured: "Tidak",
    Status: "Aktif",
  ...
  cmsData.pricing.push(newPkg);
  syncLocalAndRemote('createPricing', newPkg, "Paket baru berhasil ditambahkan!");
  renderPricingTable();
  renderOverview();
}
...
function deletePricing(id) {
  ...
  syncLocalAndRemote('deletePricing', { id }, "Paket harga dihapus!");
  renderPricingTable();
  renderOverview();
}
...
function renderFeaturesTable() {
  ...
  tbody.innerHTML = cmsData.features.map(f => `
    <tr>
...
function openFeatureModal() {
  const judul = prompt("Judul Fitur Baru:");
  if (!judul) return;
  const deskripsi = prompt("Deskripsi singkat fitur:");
  const newFeat = {
  ...
  cmsData.features.push(newFeat);
  syncLocalAndRemote('createFeature', newFeat, "Fitur berhasil ditambahkan!");
  renderFeaturesTable();
  renderOverview();
}
...
function deleteFeature(id) {
  ...
  syncLocalAndRemote('deleteFeature', { id }, "Fitur dihapus!");
  renderFeaturesTable();
  renderOverview();
}
...
function renderTestiTable() {
  ...
  tbody.innerHTML = cmsData.testimonials.map(t => `
    <tr>
...
function openTestiModal() {
  const nama = prompt("Nama Klien (misal: Fajar & Anisa):");
  if (!nama) return;
  const testimoni = prompt("Kutipan Testimoni:");
  const newTesti = {
  ...
  cmsData.testimonials.push(newTesti);
  syncLocalAndRemote('createTestimonial', newTesti, "Testimoni ditambahkan!");
  renderTestiTable();
  renderOverview();
}
...
function deleteTesti(id) {
  ...
  syncLocalAndRemote('deleteTestimonial', { id }, "Testimoni dihapus!");
  renderTestiTable();
  renderOverview();
}
...
function openHowModal() {
  const nomor = prompt("Nomor Langkah (contoh: 05):");
  if (!nomor) return;
  const judul = prompt("Judul Langkah:");
  const deskripsi = prompt("Deskripsi Langkah:");
  const newHow = { Nomor: nomor, Judul: judul, Deskripsi: deskripsi, Icon: "Send", Status: "Aktif" };
...
function editHowto(nomor) {
  ...
  if (!item) return;
  const judulBaru = prompt("Judul Langkah:", item.Judul);
  if (judulBaru === null) return;
  const descBaru = prompt("Deskripsi Langkah:", item.Deskripsi);
  item.Judul = judulBaru;
  item.Deskripsi = descBaru;
  syncLocalAndRemote('updateHowToOrder', item, "Langkah pemesanan diperbarui!");
  renderHowtoTable();
}
...
function renderFaqTable() {
  ...
  tbody.innerHTML = cmsData.faq.map(f => `
    <tr>
...
function openFaqModal() {
  const pertanyaan = prompt("Pertanyaan Baru:");
  if (!pertanyaan) return;
  const jawaban = prompt("Jawaban FAQ:");
  const newFaq = {
  ...
  cmsData.faq.push(newFaq);
  syncLocalAndRemote('createFAQ', newFaq, "FAQ baru ditambahkan!");
  renderFaqTable();
  renderOverview();
}
...
function deleteFaq(id) {
  ...
  syncLocalAndRemote('deleteFAQ', { id }, "FAQ dihapus!");
  renderFaqTable();
  renderOverview();
}
